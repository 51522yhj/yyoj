...
public abstract class CppCodeSandboxTemplate implements CodeSandbox {
    ...
    public ExecuteMessage compileFile(File userCodeFile) {
        String compileCmd = String.format("g++ -o %s %s", userCodeFile.getParentFile().getAbsolutePath() + File.separator + "main", userCodeFile.getAbsolutePath());
       log.info("compileCmd cpp = {}", compileCmd);
        try {
    ...
    public List<ExecuteMessage> runFile(File userCodeFile, List<String> inputList) {
        ...
        for (String inputArgs : inputList) {
            String commandArray = executablePath;
            log.info("inputArgs = {}", inputArgs);
            if (StrUtil.isNotBlank(inputArgs)) {
                commandArray = executablePath + " " + inputArgs;
            }
            try {
                log.info("runFile commandArray = {}", commandArray);
                Process runProcess = Runtime.getRuntime().exec(commandArray);
                // 超时控制
                new Thread(() -> {
                    try {
