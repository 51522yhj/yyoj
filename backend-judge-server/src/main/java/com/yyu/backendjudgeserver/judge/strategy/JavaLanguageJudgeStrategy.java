...
public class JavaLanguageJudgeStrategy implements JudgeStrategy {
    ...
    public JudgeInfo doJudge(JudgeContext judgeContext) {
        ...
        log.info("judgeInfoResponse1:{}",judgeInfoResponse);
        // 先判断沙箱执行的结果输出数量是否和预期输出数量相等
        if (outputList.size() != inputList.size()) {
            judgeInfoMessageEnum = JudgeInfoMessageEnum.WRONG_ANSWER;
            judgeInfoResponse.setMessage(judgeInfoMessageEnum.getValue());
            log.info("judgeInfoResponse2:{}",judgeInfoResponse);
        ...
        for (int i = 0; i < judgeCaseList.size(); i++) {
            ...
            if (!judgeCase.getOutput().equals(outputList.get(i))) {
                judgeInfoMessageEnum = JudgeInfoMessageEnum.WRONG_ANSWER;
                judgeInfoResponse.setMessage(judgeInfoMessageEnum.getValue());
                log.info("judgeInfoResponse3:{}",judgeInfoResponse);
                return judgeInfoResponse;
            }
        }
        // 判断题目限制
        ...
        if (memory > needMemoryLimit) {
            ...
            judgeInfoResponse.setMessage(judgeInfoMessageEnum.getValue());
            log.info("judgeInfoResponse4:{}",judgeInfoResponse);
            return judgeInfoResponse;
        }
        ...
        if ((time - JAVA_PROGRAM_TIME_COST) > needTimeLimit) {
            ...
            judgeInfoResponse.setMessage(judgeInfoMessageEnum.getValue());
            log.info("judgeInfoResponse5:{}",judgeInfoResponse);
            return judgeInfoResponse;
        }
        judgeInfoResponse.setMessage(judgeInfoMessageEnum.getValue());
        return judgeInfoResponse;
    ...
    @Override
    public JudgeInfo doJudge(JudgeContentCompetition judgeContext) {
        log.info("judgeContext:{}",judgeContext);
        JudgeInfo judgeInfo = judgeContext.getJudgeInfo();
        Long memory = Optional.ofNullable(judgeInfo.getMemory()).orElse(0L);
        Long time = Optional.ofNullable(judgeInfo.getTime()).orElse(0L);
        List<String> inputList = judgeContext.getInputList();
        List<String> outputList = judgeContext.getOutputList();
        CompetitionQuestion question = judgeContext.getQuestion();
        List<JudgeCase> judgeCaseList = judgeContext.getJudgeCaseList();
        JudgeInfoMessageEnum judgeInfoMessageEnum = JudgeInfoMessageEnum.ACCEPTED;
        JudgeInfo judgeInfoResponse = new JudgeInfo();
        judgeInfoResponse.setMemory(memory);
